...
function bmiText(item) {
  ...
  text = `<div class="circle">
                    ...
                    <h2 class="p-${item[0].lightbar}">${item[0].BMI.toFixed(
    2
  )}</h2>
                    <p class="p-${item[0].lightbar}">BMI</p>
                    </div>
                    <input type="button" class="icon bg-${
                      item[0].lightbar
                    } clear" >
                </div>
  ...
  if (data.length >= 1) {
    ...
    for (let i = 0; i < len; i++) {
      str += `<li>
            <table>
                <tr>
                    <td class="lightbar ${item[i].lightbar}">${
        item[i].status
      }</td>
                    <td>${item[i].BMI.toFixed(2)}</td>
                    <td> ${
                      item[i].height
                    }</td>
                    <td> ${
